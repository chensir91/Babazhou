# 改动日志 v2.21
- 2026年06月26日，v2.21：修复战斗选择逻辑——切换到已行动队友时禁用操作、敌方不可攻击时切换查看

# 改动日志 v1.56
- 2026年06月05日，选位界面重构：侧栏角色改为正方格42×42（蓝红渐变背景区分阵营）；删除地图下方角色条仅保留两侧角色；修复跨阵营放置bug（选位时只能点击已方角色+棋盘双重校验_deployCur._tm）；main-grid间隙收窄4→2px控制总宽390px

# 改动日志 v1.55
- 2026年06月05日，选人棋盘取消公共区域限制：移除中列不可放置规则，35格全可用；所有格子初始黑色背景(#111)；角色被选后原地留空不进位
